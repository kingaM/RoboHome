...
class TestMiddleLayer(unittest.TestCase):
    ...
    def test_stateChangesTriggered(self):
        db = MockDB()
        house = House(db)
        item = Openable(1, "item1", "arduino", "door", "192.168.0.100")
    ...
    def test_reactToEventTriggeredByStateChange(self):
        ...
        item = Openable(1, "item1", "arduino", "door", "192.168.0.101")
        item.subscriptionManager.addListener(house.reactToEvent)
        room = MockRoom(1, "lounge")
        room.items = {1: item}
        house.rooms = {1: room}
